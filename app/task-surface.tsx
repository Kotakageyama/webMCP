...
import { registerTaskSurfaceTools, toolNamesForTask, type WebMcpStatus } from "./webmcp";

const intents: Record<TaskKind, { label: string; copy: string }> = {
  refund: { label: "Return an item", copy: "Return only the red T-shirt from order #TS-1042" },
  shipping_update: { label: "Change delivery address", copy: "Change the delivery address for order #TS-1042" },
  shipment_cancel: { label: "Cancel shipment", copy: "Cancel the shipment for order #TS-1042" }
};
...
export default function TaskSurface({ initialOrder }: { initialOrder: Order }) {
  ...
  const [webMcpStatus, setWebMcpStatus] = useState<WebMcpStatus>({ state: "checking", toolNames: toolNamesForTask(null) });
  ...
  useEffect(() => registerTaskSurfaceTools({ task, open: setTask, onStatus: setWebMcpStatus, prepare: (kind, input) => {
    setTask(kind);
  ...
  return <main className="shell">
    <aside className="sidebar"><div className="brand"><b>◫</b> TaskSurface</div><nav>{["Orders", "Products", "Customers", "Shipments"].map((name, index) => <button className={index === 0 ? "nav active" : "nav"} key={name}>{name}</button>)}<a className="nav securityNav" href="/security-lab">Security Lab <small>日本語</small></a></nav><div className="agent"><span className="pulse" /> <div><strong>Agent Helix</strong><small>{task ? "Working in this task" : "Ready"}</small></div></div></aside>
...
function WebMcpDiagnostic({ status }: { status: WebMcpStatus }) { const message = status.state === "ready" ? `${status.toolNames.length} tools registered for this page state.` : status.state === "checking" ? "Checking this browser for WebMCP support…" : status.state === "unsupported" ? "WebMCP is not available in this browser context." : `WebMCP registration failed: ${status.message}`; return <p className={`webMcpStatus ${status.state}`}>{message}</p>; }
function Dashboard({ order, onStart, webMcpStatus }: { order: Order; onStart: (t: TaskKind) => void; webMcpStatus: WebMcpStatus }) { return <div className="dashboard"><section className="intent"><p className="eyebrow">What should we accomplish?</p><h2>Turn a live page into a shared task surface.</h2><p className="muted">Choose a purpose. TaskSurface narrows the UI to the required capabilities, then returns final judgment to you.</p><div className="intentButtons">{(Object.keys(intents) as TaskKind[]).map((key) => <button key={key} onClick={() => onStart(key)}><strong>{intents[key].label}</strong><small>{intents[key].copy}</small><span>Start →</span></button>)}</div></section><section className="orderCard"><p className="eyebrow">Current order</p>{order.items.map((item) => <div className="item" key={item.id}><div className={item.id === "item-red-shirt" ? "swatch red" : "swatch"} /><div><strong>{item.title}</strong><small>{item.variant} · Qty {item.quantity}</small></div><b>{yen(item.unitPriceYen)}</b></div>)}<div className="addressSummary"><p className="eyebrow">Delivery</p><strong>{order.shippingAddress.recipient}</strong><span>{order.shippingAddress.line1}, {order.shippingAddress.city}</span></div><p className="eyebrow webMcpLabel">Live WebMCP tools</p><div className="tools">{webMcpStatus.toolNames.map((tool) => <code key={tool}>{tool}</code>)}</div><WebMcpDiagnostic status={webMcpStatus} /></section></div>; }
